...
public class GetBranchOfficesResponseConverter extends ResponseConverter<GetBranchOfficesResponse, List<BranchOfficeEntity>> {
    ...
    public List<BranchOfficeEntity> convert(GetBranchOfficesResponse response) {
        ...
        for (GetBranchOfficesResponse.BranchOffice branchOffice : response.getBranchOffices()) {
            //Descartar sucursal 8000 (Corporativo),  9999 (Laudos), 9997 (Infonavit) y 3000 (AforeMovil)
            if (!branchOffice.getBranchOfficeId().equals(Constants.ID_BRANCH_CORPORATE)
                && !branchOffice.getBranchOfficeId().equals(Constants.ID_BRANCH_LAUDOS)
                && !branchOffice.getBranchOfficeId().equals(Constants.ID_BRANCH_INFONAVIT)
                && !branchOffice.getBranchOfficeId().equals(Constants.ID_BRANCH_AFOMOV)
            ){
